// src/components/Projects/DigitalMarketingProjectsSection.jsx
import { digitalMarketingProjects } from "../../data/digitalMarketingProjects";
      ...
      <div className="mx-auto max-w-7xl px-6 py-12 md:py-16">
        ...
        >
          ...
          <p className="mt-4 text-sm md:text-base text-black leading-relaxed">
            Campaigns, CRO, analytics, and content systems — built with premium clarity and real outcomes.
          </p>

          <div className="mt-7 h-px w-full bg-gradient-to-r from-black via-black to-transparent" />
        </div>

        {/* ===== Grid: 6 boxes (3 + 3 on desktop) ===== */}
        <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {digitalMarketingProjects.slice(0, 6).map((p, idx) => (
            <a
              key={p.id}
              href={p.url}
              target="_blank"
            ...
            >
              <article
                className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] backdrop-blur-xl
                           ...
                           hover:-translate-y-1"
              >
                {/* IMAGE (NO overlay, fully visible) */}
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className="rounded-full border border-white/10 bg-black px-3 py-1 text-[11px] text-white/90
                                 transition-colors duration-300 group-hover:border-rose-300/30 group-hover:text-white"
                    >
                      Open ↗
                    </span>
                  </div>
                ...
                {/* Premium hover glow (no image overlay) */}
                <div
                  className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                  style={{
                    boxShadow:
                      "inset 0 0 0 1px rgba(244,63,94,0.18), 0 22px 90px rgba(0,0,0,0.35)",
                  }}
                />
              </article>
            </a>
